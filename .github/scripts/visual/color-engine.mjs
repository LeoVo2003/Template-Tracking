...
export function brandEvidence(samples) {
  const roles = { social: 0, cta: 3.6, button: 3.2, active_nav: 3.1, token_accent: 2.6, control: 2.1, heading: 1.8, border: 1.55, icon: 1.35, decorative: 1.25, card: 0.85, nav_link: 0.45, ordinary_link: 0.18, text: 0.10, section: 0.08, canvas: 0.03 };
  const rows = Object.fromEntries(FAMILIES.map((family) => [family, { family, score: 0, roles: new Set(), sections: new Set(), controls: new Set(), coverage: 0, hex: '', sources: [] }]));
  const seen = new Set();
  for (const sample of samples || []) {
    ...
    if (row.sources.length < 12) row.sources.push({ role, kind: sample.kind || '', section_key: sample.section_key || 'body', control_key: sample.control_key || '', href_host: sample.href_host || '', href_path: sample.href_path || '', class_hint: sample.class_hint || '', contribution: Number(contribution.toFixed(5)) });
  }
  return Object.values(rows).map((row) => ({ ...row, score: Number((row.score * (1 + Math.min(0.42, Math.max(0, row.sections.size - 1) * 0.08)) * (1 + Math.min(0.26, Math.max(0, row.roles.size - 1) * 0.06))).toFixed(4)), coverage: Number(row.coverage.toFixed(4)), roles: [...row.roles], section_count: row.sections.size, control_count: row.controls.size, sources: row.sources.sort((a, b) => b.contribution - a.contribution).slice(0, 6) })).sort((a, b) => b.score - a.score);
}
...
export async function analyzeUiColor(previewBuffer, semanticSamples = []) {
  ...
  const neutralControlFamilies = new Set(['greige', 'gray', 'charcoal', 'beige', 'taupe']);
  const candidates = brandEvidence(semanticSamples).filter((entry) => {
    if (['white', 'ivory', 'cream', 'black', 'neutral'].includes(entry.family)) return false;
    if (!neutralControlFamilies.has(entry.family)) return true;
    const meaningfulRoles = entry.roles.filter((role) => !['cta', 'button', 'control'].includes(role));
    return meaningfulRoles.length > 0 || (entry.section_count >= 3 && entry.control_count >= 3 && entry.roles.length >= 2);
  });
